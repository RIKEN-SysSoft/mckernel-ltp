...
i=0
$LTPMCEXEC ./pthread_cond_many --realtime --broadcast -i $iter -n $nthread > $nthread.$iter.$nproc.$i.out &
i=1
while test $i -lt $nproc
do
	if [ -n "$LTPMCEXEC" ]; then
		$MCEXEC_HOOK "$LTPMCEXEC ./pthread_cond_many --broadcast -i $iter -n $nthread > $nthread.$iter.$nproc.$i.out" &
	else
